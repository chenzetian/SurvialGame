...
public class PlayerListener implements Listener {
	...
	public void onPlayerInteractEvent(PlayerInteractEvent event) {
		if(event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK) {
			...
			if(hand.equals(Game.getJobItems())) {
				Playerstate=1;
				p.sendMessage(ChatColor.RED + "你选择了战士");
				p.getInventory().clear(2);
				p.getInventory().clear(4);
				p.getInventory().clear(6);
				p.getInventory().setItem(1,Game.getitems() );
			}
			if(hand.equals(Game.getJobItemshe())) {
				Playerstate=2;
				p.sendMessage(ChatColor.RED + "你选择了射手");
				p.getInventory().clear(2);
				p.getInventory().clear(4);
				p.getInventory().clear(6);
			}
			if(hand.equals(Game.getjobItemTh())) {
				Playerstate=3;
				p.sendMessage(ChatColor.RED + "你选择了强盗");
				p.getInventory().clear(2);
				p.getInventory().clear(4);
				p.getInventory().clear(6);
			}
