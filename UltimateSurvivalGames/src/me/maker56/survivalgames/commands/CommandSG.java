...
public class CommandSG implements CommandExecutor {
	...
	public CommandSG() {
		this.authors = " �-- Plugin developed by ";
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
	    ...
	    {
	      if (args.length == 0) {
	        sender.sendMessage(MessageHandler.getMessage("prefix") + "Version " + SurvivalGames.instance.getDescription().getVersion() + this.authors);
	        ...
	        if (PermissionHandler.hasPermission(sender, Permission.CONFIG))
	          sender.sendMessage("§8/§6sg config §7- §eShows the configuration management helpsite!");
				...
				// LIST
					...
					sender.sendMessage(MessageHandler.getMessage("prefix") + "List of all loaded lobbys�: �(" + games.size() + "�)");
					for(Game game : games) {
						sender.sendMessage("�- �" + game.getName() + "�: " + game.getState().toString() + " �(" + game.getPlayingUsers() + "�/" + game.getMaximumPlayers() + "�)");
					}
					return true;
				...
				// DEBUG
					...
					Util.debug = nV;
					sender.sendMessage(MessageHandler.getMessage("prefix") + "Debug Mode�: " + (nV ? "ENABLED" : "DISABLED"));
					return true;
					
				}
				
				sender.sendMessage(MessageHandler.getMessage("prefix") + "Command not found! Type /sg for help!");
				return true;
			}
